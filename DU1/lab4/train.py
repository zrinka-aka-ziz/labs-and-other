...
if __name__ == '__main__':
    ...
    ds_train2 = MNISTMetricDataset(mnist_download_root, split='train', remove_class=0) #used for removing the class 0
    ...
    train_loader2 = DataLoader( #train loader for the dataset minus the 0
        ds_train2,
        batch_size=64,
        shuffle=True,
        pin_memory=True,
    ...
    for epoch in range(epochs):
        print(f"Epoch: {epoch}")
        t0 = time.time_ns()
        train_loss = train(model, optimizer, train_loader2, device) #no 0 for training
        ...
        if EVAL_ON_TEST or EVAL_ON_TRAIN:
            ...
            representations = compute_representations(model, train_loader, num_classes, emb_size, device) #other classes are OK for reps
        if EVAL_ON_TRAIN:
    ...
    torch.save(model.state_dict(), 'model_removed0.pt')#save the model
