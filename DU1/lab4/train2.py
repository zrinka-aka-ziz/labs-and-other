...
if __name__ == '__main__':
    ...
    emb_size = 784 #28*28
    model = IdentityModel().to(device) #Identity Model
    ...
    #t0 = time.time_ns()
        ...
        #representations = compute_representations(model, train_loader2, num_classes, emb_size, device)
    #if EVAL_ON_TRAIN:
        #print("Evaluating on training set...")
        #acc1 = evaluate(model, representations, traineval_loader, device)
        #print(f"Epoch {epoch}: Train Top1 Acc: {round(acc1 * 100, 2)}%")
    #if EVAL_ON_TEST:
    print("Evaluating on test set...") #only does the test set since it can't be trained
